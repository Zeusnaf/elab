...
if(isset($_POST['login'])){
    ...
    if($category=="labtechnician"){
        $sel="SELECT * FROM users WHERE user_type='labtechnician'AND username='$username' AND password='$password' AND status='active'";
        ...
        $sel="SELECT * FROM users WHERE user_type='secretary' AND username='$username' AND password='$password' AND status='active'";
        $select=$connect->query($sel);
        $count=$select->num_rows;
